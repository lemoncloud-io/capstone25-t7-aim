...
export const getCityCountryCorrection = async (
    ...
    const prompt = `You are a geographical spelling correction expert. Given the user input for a city: "${city}" and a country: "${country}", correct any spelling mistakes. If the input seems correct, return the original values. Determine if a correction was made. Respond ONLY in the specified JSON format.`;
    ...
    try {
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: {
                    type: Type.OBJECT,
                    properties: {
                        correctedCity: { type: Type.STRING, description: 'The corrected city name.' },
                        correctedCountry: { type: Type.STRING, description: 'The corrected country name.' },
                        needsCorrection: { type: Type.BOOLEAN, description: 'True if a correction was made.' },
                    },
                    required: ['correctedCity', 'correctedCountry', 'needsCorrection'],
                },
            },
        });
        ...
        // If the corrected text is the same as the original, it doesn't need correction
        if (
            result.correctedCity.toLowerCase() === city.toLowerCase().trim() &&
            result.correctedCountry.toLowerCase() === country.toLowerCase().trim()
        ) {
        ...
        console.error('Error getting city/country correction:', error);
